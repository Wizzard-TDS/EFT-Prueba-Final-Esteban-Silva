...
def stock_plataforma(plataforma, juegos, inventario):
    ...
    for cod, datos_juego in juegos.items():
        # datos_juego[1] corresponde a la plataforma
        if datos_juego[1].strip().lower() == plataforma_target:
            if cod in inventario:
                # inventario[cod][1] corresponde al stock
...
def eliminar_juego(codigo, juegos, inventario):
    """
    Opción 5: Elimina un juego de ambos diccionarios.
    Retorna True si eliminó correctamente, False si no existe.
    """
    cod_real = buscar_codigo(codigo, juegos)
    if cod_real is None:
        return False
...
def main():
    ...
    while True:
        ...
        if opcion == '1':
            ...
            mult_str = input("¿Es multiplayer? (s = si y n = no): ")
